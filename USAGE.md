...
1. [安装](#安装)
2. [BaseEntity - 实体基类](#baseentity---实体基类)
...
14. [跨视图导航契约（view.navigation 包）- 跨视图传数据解耦](#跨视图导航契约viewnavigation-包--跨视图传数据解耦)
15. [视图基类（view.base 包）](#视图基类viewbase-包)
16. [Jmix 框架翻译补充](#jmix-框架翻译补充)
...
- **与 TAB 模式正交**：TAB 模式（标签页内编辑）的同等守卫在多标签页布局插件的 `TabDetailProcessor` 中，两者互不影响。
...
> 这是一套**隐式字符串契约**：消费方是多标签页布局插件的 `TabDetailProcessor`，由它完成保存后的列表回填。
>
> - **安装了多标签页布局插件**：完整 TAB 回填行为生效；
> - **未安装多标签页布局插件**：Action 仍可正常打开详情，附加的参数/handler 无人消费，无害退化为标准导航。

即：这三个 Action 可独立于多标签页布局插件使用；TAB 模式的增强回填需要多标签页布局插件配合。
...
标准 `list_create` / `list_edit` 在 DIALOG 模式下的关闭回调时序不统一、缺少"关闭即可中断"的入口。本接口定义四阶段时序，Phase 1-3 由增强 Action（DIALOG 模式）或多标签页布局插件的 `TabDetailProcessor`（TAB 模式）触发，Phase 4 由 Action 或 `ComponentUtil` 桥接触发。
...
「视图生命周期事件 + 展示模式/行为声明」契约，位于 `org.magic.jmix.addons.core.tab` 包。

### 背景 / 为什么放在 core

多标签页布局插件提供多标签页架构，但它是**宿主框架级**的能力，其他 addon 不应直接依赖它。为了让任意 addon 能声明「我享受多标签特性」又保持松耦合，把跨 addon 共用的契约放在 core：

- 使用方只依赖 core，贴注解 / 实现接口即可声明多标签特性；
- 普通宿主（未安装多标签页布局插件）中：注解被忽略、事件不触发，但**无副作用、无报错**，视图正常工作；
- 安装了多标签页布局插件的宿主中：自动获得多标签增强（标签页打开、激活/停用事件、不可关闭/多开声明等）。
...
| 场景 | 行为 |
|------|------|
| 标签页切换（安装多标签页布局插件） | `TabManager.activateTab()` 先 fire `TabDeactivateEvent`（旧视图），再 fire `TabActivateEvent`（新视图） |
| 标签页关闭 | 不触发激活/停用事件，走标准的 `BeforeCloseEvent` / `AfterCloseEvent` |
| 普通宿主（未安装多标签页布局插件） | 事件不触发，但接口方法与注解均无害 |

### 谁应该使用

- **需要多标签页特性的 addon 视图**：依赖 core，实现 `TabActivationAware`、贴展示模式/行为注解即可，不感知多标签页布局插件是否存在。
- **宿主项目视图**：同上。宿主若安装了多标签页布局插件，标签页系统的 `TabManager` 会读取这些契约并提供增强。

> 这套契约的**运行时实现**（标签页创建、激活、路由拦截、URL 同步等）仍在多标签页布局插件的 `TabManager` / `TabRouterLayout` 等类中。core 只提供契约，不提供多标签运行时。

---

## 跨视图导航契约（view.navigation 包） - 跨视图传数据解耦

跨视图传数据的解耦方案，位于 `org.magic.jmix.addons.core.view.navigation` 包。

### 背景 / 为什么放在 core

多标签页架构下，Jmix 原生的 `viewNavigators.view().withAfterNavigationHandler()` 会失效（回调依赖 origin view detach，多标签页下 view 不 detach）。替代方案是 `tabRouterService.open().withAfterViewCreated()`，但这需要依赖 tab-layout addon，违反插件间不依赖原则。

通过策略模式解耦：Core addon 定义 `ViewNavigationSupport` 契约接口 + 默认实现（走 Jmix viewNavigators），Tab-layout addon 的 `TabRouterService` 实现该接口并提供覆盖 Bean。任何依赖 Core addon 的模块注入 `ViewNavigationSupport` 即可跨视图传数据，**无需直接依赖 tab-layout addon**。
...
| 场景 | 注入的实现 | 说明 |
|------|-----------|------|
| 引入 tab-layout addon | `TabRouterService` | 多标签页架构下回调正常工作 |
| 未引入 tab-layout addon | `DefaultViewNavigationSupport` | 回退到 Jmix 原生 `viewNavigators`（单标签页下正常，多标签页下 `withAfterNavigationHandler` 失效） |

### ViewNavigationBuilder API

| 方法 | 说明 |
|------|------|
| `withAfterViewCreated(Consumer<V>)` | 一次性回调，在视图创建后、BeforeShow 前触发 |
| `withAfterViewClosed(Consumer<AfterViewClosedEvent<V>>)` | 一次性关闭回调，在目标视图 AfterCloseEvent 触发时执行 |
| `navigate()` | 执行导航，触发路由 |

### AfterViewClosedEvent 属性

| 方法 | 说明 |
|------|------|
| `getView()` | 已关闭的视图实例 |
| `getCloseAction()` | 关闭动作，与 `View.AfterCloseEvent.getCloseAction()` 一致 |
| `closedWith(StandardOutcome)` | 检查视图是否以指定结果关闭 |

### 限制

`viewNavigationSupport.open()` 仅支持无参数路由（如 `@Route("users")`），不支持带路径参数的路由（如 `@Route("user/:id")`）。带参数路由请使用 `viewNavigators.detailView().navigate()`。

### Tab-layout addon 覆盖实现

| 类 | 说明 |
|----|------|
| `TabRouterService` | 实现 `ViewNavigationSupport`，`open()` 返回 `TabViewNavigator` |
| `TabViewNavigator<V>` | 继承 `ViewNavigationBuilder<V>`，覆盖 `navigate()` 走多标签页路由 |

### 谁应该使用

- **需要跨视图传数据的 addon**：依赖 core，注入 `ViewNavigationSupport` 即可，无需直接依赖 tab-layout
- **宿主项目**：注入 `ViewNavigationSupport` 或直接注入 `TabRouterService` 均可（后者功能更多，包含标签页管理、Drawer 控制等）

---

## 视图基类（view.base 包）

列表/详情视图基类，位于 `org.magic.jmix.addons.core.view.base`。
...
> `BaseListView` / `BaseDetailView` 与多标签事件契约、`DetailViewCloseCallback` 配合，在安装了多标签页布局插件的宿主中获得完整的 TAB 模式 CRUD 时序；在普通宿主中作为标准视图基类正常工作。

---

## Jmix 框架翻译补充

补充 Jmix 2.8 框架未翻译的中文消息，安装 core addon 后自动生效。
...
- 新增 MagicDetailWindowBuilderProcessor：`@Primary` 替换框架 `DetailWindowBuilderProcessor`，修复 Dialog 模式未持久化时脏数据回写列表
- 新增强 Action：`magic_list_create` / `magic_list_edit` / `magic_list_remove`，统一 TAB/DIALOG 关闭回调时序、兼容 Container/MetaClass 两种 Grid 模式
- 新增 DetailViewCloseCallback 详情关闭回调契约接口（四阶段：onClose/onSaved/afterSaved/afterSaveHandler）
- 新增多标签契约（`tab` 包）：`TabActivationAware` / `TabActivateEvent` / `TabDeactivateEvent`（标签页激活/停用事件契约）、`@PresentationModes` / `PresentationMode` / `PresentationModeHelper`（展示模式声明）、`@UncloseableTab` / `@MultipleOpenTab`（标签页行为声明）。让其他 addon 仅依赖 core 即可声明多标签特性，普通宿主中无副作用，安装多标签页布局插件后自动获得增强。
- 新增视图基类 `BaseListView` / `BaseDetailView`（`org.magic.jmix.addons.core.view.base`）。`BaseListView` 提供 `DetailViewCloseCallback` 四阶段关闭回调默认实现；`BaseDetailView` 提供自动标题与保存状态追踪；自动标题消息 `detailView.title.new/edit/view` 一并提供。
- 新增 `CursorLazyGrid` 游标分页懒加载 Grid 组件（`CursorLazyGrid` / `CursorPagedDataProvider` / `CursorPage` / `CursorPageFetcher`），将游标顺序分页后端适配为 Grid 原生滚动懒加载，支持 undefined size 模式与异步 `enablePushUpdates`；注意其 DataProvider 为**全量缓存**（只增不减），大数据场景需评估内存。
- 新增跨视图导航契约（`view.navigation` 包）：`ViewNavigationSupport` 契约接口 + `ViewNavigationBuilder<V>` Builder 基类 + `AfterViewClosedEvent<V>` 关闭事件 + `DefaultViewNavigationSupport` 默认实现。解决多标签页架构下 `withAfterNavigationHandler` 失效且 addon 不能依赖 tab-layout 的问题；安装 tab-layout 后 `TabRouterService` 自动覆盖默认实现。
