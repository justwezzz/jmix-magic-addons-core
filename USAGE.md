...
多标签页架构下，Jmix 原生的 `viewNavigators.view().withAfterNavigationHandler()` 会失效（回调依赖 origin view detach，多标签页下 view 不 detach）。Core addon 定义 `ViewNavigationSupport` 契约接口 + 默认实现（走 Jmix viewNavigators），其他插件可通过实现该接口提供覆盖实现。使用方注入 `ViewNavigationSupport` 即可跨视图传数据，通过契约接口获得更好的扩展性。
...
- **在多标签页架构中需要跨视图传数据的视图**：**必须**使用 `ViewNavigationSupport`。Jmix 原生的 `viewNavigators.view().withAfterNavigationHandler()` 在多标签页架构下会失效，`ViewNavigationSupport` 是唯一可靠的跨视图传数据方式。
- **不在多标签页架构中的视图**：同样推荐使用 `ViewNavigationSupport`。默认实现走 Jmix viewNavigators，功能等价；同时通过契约接口获得扩展性——将来安装多标签页布局插件后无需改代码即可自动获得增强。

> 这套契约的**运行时实现**（多标签页架构下的回调能力等）由其他插件提供。Core 只提供契约和默认实现，不提供多标签页架构下的运行时。
...
- 新增跨视图导航契约（`view.navigation` 包）：`ViewNavigationSupport` 契约接口 + `ViewNavigationBuilder<V>` Builder 基类 + `AfterViewClosedEvent<V>` 关闭事件 + `DefaultViewNavigationSupport` 默认实现。解决多标签页架构下 `withAfterNavigationHandler` 失效的问题；其他插件可通过实现 `ViewNavigationSupport` 接口提供覆盖实现。
